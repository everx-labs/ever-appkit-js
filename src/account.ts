...
export class Account {
    ...
    private static async createGiver(client: TonClient): Promise<AccountGiver> {
        ...
        const giverContract = getDefaultGiverContract(getEnv("EVERCLOUD_GIVER_TYPE") ?? "v3");
        const giverAddress = await getDefaultGiverAddress(client, giverKeys, giverContract)
        ...
        return {
            ...
            sendTo: async (address, value) => {
                const topup = await giver.run("sendTransaction", {
                    dest: address,
                    value,
                    bounce: false,
                })
